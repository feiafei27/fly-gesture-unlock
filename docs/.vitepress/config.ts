...
export default defineConfig({
  title: "Fly Gesture Unlock 官网",
  description: "手势解锁库",
  srcDir: 'src',
  head: [['link', { rel: 'icon', href: '/assets/head.png' }]],
  themeConfig: {
    sidebar: {
      '/guide/': [
        {
          ...
          items: [
            ...
            { text: '手势解锁渲染器', link: '/guide/gesture-unlock-renderer' },
            { text: 'anchorDefines：锚点配置项', link: '/guide/anchor-defines' },
            { text: 'anchorStatusStyles：锚点状态样式配置项', link: '/guide/anchor-status-styles' },
            { text: 'lineStatusStyles：锚点连线状态样式配置项', link: '/guide/line-status-styles' },
            { text: 'events：定义事件回调', link: '/guide/events' },
            { text: 'config：配置业务方面的各种细节', link: '/guide/config' },
          ]
        }
      ],
    },
    ...
    socialLinks: [
      {
        icon: 'github',
        link: 'https://github.com/feiafei27/fly-gesture-unlock'
      },
      {
        icon: {
        ...
        link: 'https://gitee.com/fei_fei27/fly-gesture-unlock'
      },
    ],
